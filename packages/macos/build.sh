...
for size in 16 32 64 128 256 512; do
  ...
  if command -v sips >/dev/null 2>&1; then
    sips -z "$size" "$size" "$ICON_SRC" --out "$sfile" >/dev/null 2>&1
    [ "$size" -le 256 ] && sips -z "$s2" "$s2" "$ICON_SRC" --out "$s2file" >/dev/null 2>&1
  else
...
if command -v iconutil >/dev/null 2>&1; then
  ...
  [ -f "$ICON_SRC" ] && cp "$ICON_SRC" "$APP/Contents/Resources/AppIcon.png" || true
fi
